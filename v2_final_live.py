...
list_candidates = ["Trump", "Clinton", "Johnson", "Stein", "McMullin",
                    "Castle", "white"]

trace_3 = Bar(x=list_candidates, y=[0, 0, 0, 0, 0, 0, 0],
              xaxis='x1', yaxis='y1',
              marker=dict(color=['rgb(186,58,51)', 'rgb(68,94,150)'
                                 'rgb(255,165,0)', 'rgb(0,255,127)'
                                 'rgb(255,255,0)', 'rgb(153,50,204)'
                                 'rgb(169,169,169)']),
...
def lets_stream():
    ...
    nb_electoral_demo = 0
    nb_electoral_repu = 0
    count_candidates  = np.zeros(len(list_candidates))
        ...
        if "state" in vote_results.columns:
            ...
            for s, w, e, c in zip(l_states, winners, electoral, vote_count):
                ...
                for i, cand in enumerate(list_candidates):
                    c = state_df.loc[state_df["vote_result"] == cand]["total"]
                    if len(c) != 0:
                        count_candidates[i] += c.values[0]

                s_map.write(dict(type="choropleth", z=new_z, zauto=False,))
                s_bar_pop.write(dict(type="bar", y=count_candidates))
                s_pie_pop.write(dict(type="pie", labels=["Trump", "Clinton"],
                                     values=[count_candidates[0],
                                             count_candidates[1]]))
